...
const QUIZ_SECTION_CONFIGS = {
  ...
  marketing: {
    ...
    placeholderText: 'Тесты по маркетингу будут добавлены в этот раздел позже. Система доступа и отдельный тип ключа уже подготовлены.'
  }
};
...
function ensurePageTransitionLoader() {
  ...
  loader.innerHTML = `
    <div class="page-transition-panel">
      <div class="page-transition-scene" aria-hidden="true">
        <div class="page-transition-quiz-card hidden-right no-transition">
          <div class="page-transition-quiz-name">Sayfiddinov<span>™</span></div>
          <div class="page-transition-quiz-meta"></div>
          <div class="page-transition-quiz-rule"></div>
          <div class="page-transition-quiz-lines">
            <div class="page-transition-quiz-line line-wide"></div>
            <div class="page-transition-quiz-line line-short"></div>
          </div>
          <div class="page-transition-quiz-option quiz-option--danger"><div class="page-transition-option-fill fill-wide"></div></div>
          <div class="page-transition-quiz-option quiz-option--neutral"><div class="page-transition-option-fill fill-medium"></div></div>
          <div class="page-transition-quiz-option quiz-option--neutral"><div class="page-transition-option-fill fill-medium"></div></div>
          <div class="page-transition-quiz-option quiz-option--success"><div class="page-transition-option-fill fill-long"></div></div>
          <div class="page-transition-nav page-transition-nav--single">
            <div class="page-transition-nav-btn"><div class="page-transition-arrow arrow-right"></div></div>
          </div>
          <div class="page-transition-nav page-transition-nav--double" style="display:none;">
            <div class="page-transition-nav-btn"><div class="page-transition-arrow arrow-left"></div></div>
            <div class="page-transition-nav-btn"><div class="page-transition-arrow arrow-right"></div></div>
          </div>
        </div>
      </div>
...
function showPageTransitionLoader(targetHref = '', options = {}) {
  ...
  startPageTransitionCardAnimation(loader);
...
function hasStoredPremiumAccessForUser(userId, requiredSection = CURRENT_QUIZ_SECTION) {
  ...
  return hasSectionAccess(record.accessType, requiredSection);
}
...
function applyPremiumAccessOverlayContent(accessStatus = {}) {
  ...
  if (leadNode) {
    leadNode.textContent = normalizedUserId
      ? 'Введите ключ для активации доступа. Типы ключей: оба раздела, только микроэкономика или только маркетинг. Срок действия — 30 дней.'
      : 'По вопросам активации доступа: @SayfiddinovM · Sayfiddinov™.';
  }
...
async function getCurrentBanStatus() {
  ...
  const accessRecord = normalizedUserId ? getStoredPremiumAccessRecordForUser(normalizedUserId) : null;
  const hasAccess = normalizedUserId ? hasStoredPremiumAccessForUser(normalizedUserId, CURRENT_QUIZ_SECTION) : false;
  ...
  return {
    ...
    availableSections: accessRecord?.sections || []
  };
}
...
function isAppMenuItemAccessible(item) {
  ...
  const requiredSection = String(item.requiredSection || '').trim().toLowerCase();
  if (!requiredSection) return true;

  const currentUserId = getCurrentKnownTelegramUserId();
  const accessRecord = getStoredPremiumAccessRecordForUser(currentUserId);
  if (!accessRecord) {
    return requiredSection === CURRENT_QUIZ_SECTION;
  }

  return hasSectionAccess(accessRecord.accessType, requiredSection);
}
